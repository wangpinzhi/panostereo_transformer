...
class RegressionHead(nn.Module):
    ...
    def _upsample(self, x: NestedTensor, disp_pred: Tensor, occ_pred: Tensor, scale: int):
        ...
        # expand dim nxhxw -> nx1xhxw
        disp_pred = torch.unsqueeze(disp_pred, dim=1)
        occ_pred = torch.unsqueeze(occ_pred, dim=1)

        # scale disparity
        disp_pred_attn = disp_pred * scale

        # upsample
        disp_pred = F.interpolate(disp_pred_attn, size=(h, w), mode='nearest')  # N x 1 x H x W
        occ_pred = F.interpolate(occ_pred, size=(h, w), mode='nearest')  # N x 1 x H x W
